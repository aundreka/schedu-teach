...
import { todayIso } from "../../../lib/deped-calendar";
import {
  getVersionHistory,
  loadRuntimeData,
  persistRebalance,
...
export default function PlanDetailScreen() {
  ...
  const tabWidth = (windowWidth - Spacing.lg * 2) / 2;
  ...
  const [reqCounts, setReqCounts] = useState<{ written_work: number; performance_task: number; exams: number } | null>(null);
  const [reqBusy, setReqBusy] = useState(false);
  const reqScale = useSharedValue(1);
  const reqScaleStyle = useAnimatedStyle(() => ({ transform: [{ scale: reqScale.value }] }));
  const tabUnderlineStyle = useAnimatedStyle(() => ({ transform: [{ translateX: tabUnderlineX.value }] }));

  // Toast
  const [toast, setToast] = useState<{ visible: boolean; message: string; variant: "success" | "error" | "info" }>({ visible: false, message: "", variant: "success" });
  ...
  // Switch tabs with spring underline
  const switchTab = useCallback((tab: "details" | "history") => {
    setActiveTab(tab);
    tabUnderlineX.value = withSpring(tab === "details" ? 0 : tabWidth, { damping: 18, stiffness: 220 });
    if (tab === "history" && versions.length === 0 && planId) {
      setVersionsLoading(true);
      getVersionHistory(planId)
        .then(setVersions)
        .catch(() => {})
        .finally(() => setVersionsLoading(false));
    }
  }, [tabUnderlineX, tabWidth, versions.length, planId]);

  // Reload versions after a restore
  const reloadVersions = useCallback(() => {
    if (!planId) return;
    getVersionHistory(planId).then(setVersions).catch(() => {});
  }, [planId]);

  // Load requirement counts from blocks table
  const loadReqCounts = useCallback(async () => {
    ...
    const { data } = await supabase
      ...
      .eq("lesson_plan_id", planId);
    if (!data) return;
    const ww = data.filter((b: any) => b.session_category === "written_work").length;
    const pt = data.filter((b: any) => b.session_category === "performance_task").length;
    const ex = data.filter((b: any) => b.session_category === "exam").length;
    setReqCounts({ written_work: ww, performance_task: pt, exams: ex });
  }, [planId]);

  useEffect(() => {
    if (planId) loadReqCounts();
  }, [planId, loadReqCounts]);

  // Adjust a requirement by delta (±1), rebalance schedule
  const adjustRequirement = useCallback(async (field: "written_work" | "performance_task", delta: number) => {
    if (!planId || reqBusy || !reqCounts) return;
    const current = reqCounts[field];
    const next = Math.max(0, current + delta);
    if (next === current) return;

    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
    reqScale.value = withSequence(withSpring(1.18, { damping: 10 }), withSpring(1, { damping: 14 }));
    setReqBusy(true);
    try {
      const data = await loadRuntimeData(planId);
      await snapshotVersion(planId, "requirements_changed", `Changed ${field.replace(/_/g, " ")} count from ${current} to ${next}`, data.slots, data.blocks);
      ...
      if (delta > 0) {
        ...
        const toDelete = data.blocks
          .filter((b) => b.session_category === field)
          .sort((a, b) => (a.slot_key ? 1 : -1));
        const victim = toDelete[toDelete.length - 1];
        if (victim?.block_id) {
          await supabase.from("blocks").delete().eq("block_id", victim.block_id);
        }
      }

      const fresh = await loadRuntimeData(planId);
      const result = rebalanceDay({ slots: fresh.slots, blocks: fresh.blocks, lessons: fresh.lessons, activities: fresh.activities, rules: fresh.rules });
      await persistRebalance(fresh, result);

      setReqCounts((prev) => prev ? { ...prev, [field]: next } : prev);
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
      showToast(`${field === "written_work" ? "Written Work" : "Performance Task"} updated to ${next}`, "success");
    } catch (err: any) {
      showToast(err?.message ?? "Could not update requirements.", "error");
    } finally {
      setReqBusy(false);
    }
  }, [planId, reqBusy, reqCounts, reqScale, showToast]);

  // Restore a version snapshot
  const confirmRestore = useCallback((entry: VersionEntry) => {
    Alert.alert(
      "Restore this version?",
      `This will revert your schedule to version ${entry.versionNo}. The current state will be saved as a new snapshot first.`,
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          style: "destructive",
          onPress: async () => {
            if (!planId || restoringId) return;
            setRestoringId(entry.versionId);
            try {
              await restoreVersion(planId, entry.versionId);
              Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
  ...
  }, [planId, restoringId, showToast, reloadVersions]);
          ...
          {plan ? (
            <View style={[styles.tabBar, { borderColor: c.border }]}>
              {(["details", "history"] as const).map((tab) => (
                <Pressable key={tab} style={styles.tabItem} onPress={() => switchTab(tab)}>
                  <Text style={[styles.tabLabel, { color: activeTab === tab ? c.tint : c.mutedText }]}>
                    {tab === "details" ? "Details" : "History"}
                  </Text>
                </Pressable>
              ))}
              <Animated.View
                style={[styles.tabUnderline, { backgroundColor: c.tint, width: tabWidth }, tabUnderlineStyle]}
              />
            </View>
          ) : null}
          ...
          {!plan ? (
            ...
            <Animated.View entering={FadeInDown.duration(220).springify()}>
              {versionsLoading ? (
                ...
                <View style={[styles.emptyState, { borderColor: c.border, backgroundColor: cardBg }]}>
                  <Ionicons name="time-outline" size={28} color={mutedText} />
                  <Text style={[styles.emptyText, { color: mutedText }]}>No history yet.</Text>
                  <Text style={[styles.emptySubText, { color: mutedText }]}>Suspensions, restores, and requirement changes appear here.</Text>
                </View>
              ) : (
                <View style={styles.timeline}>
                  {versions.map((entry, i) => {
                    ...
                    return (
                      <Animated.View key={entry.versionId} entering={FadeInDown.delay(i * 40).springify()} style={styles.timelineRow}>
                        <View style={styles.timelineLeft}>
                          <View style={[styles.timelineDot, { borderColor: c.tint, backgroundColor: cardBg }]} />
                          {!isLast ? <View style={[styles.timelineLine, { backgroundColor: c.border }]} /> : null}
                        </View>
                        <View style={[styles.timelineCard, { backgroundColor: cardBg, borderColor: c.border }]}>
                          <View style={styles.timelineCardTop}>
                            ...
                            <Text style={[styles.timelineDate, { color: mutedText }]}>
                              {formatRelativeDate(entry.createdAt)}
                            </Text>
                          </View>
                          {entry.triggerDescription ? (
                            <Text style={[styles.timelineDesc, { color: c.text }]}>{entry.triggerDescription}</Text>
                          ) : null}
                          <Text style={[styles.timelineStats, { color: mutedText }]}>
                            v{entry.versionNo} · {entry.blocksSnapshot.length} blocks · {entry.slotsSnapshot.length} slots
                          </Text>
                          {entry.triggerType !== "manual_restore" ? (
                            <Pressable
                              style={[styles.restoreBtn, { borderColor: c.tint, opacity: isRestoring ? 0.6 : 1 }]}
                            ...
                            >
                              {isRestoring ? (
                                ...
                                <>
                                  <Ionicons name="arrow-undo" size={12} color={c.tint} />
                                  <Text style={[styles.restoreBtnText, { color: c.tint }]}>Restore</Text>
                                </>
                              )}
                            </Pressable>
                          ) : null}
              ...
              <View style={[styles.heroCard, { backgroundColor: cardBg, borderColor: c.border }]}>
                ...
                <View style={styles.heroMetaRow}>
                  ...
                  {(() => {
                    ...
                    const weekLabel = formatTermProgress(
                      [{ term_no: 1, label: "Plan", start_date: plan.start_date, end_date: plan.end_date }],
                      today,
                    );
                    return weekLabel ? (
                      <View style={[styles.badge, { backgroundColor: c.tint + "1A" }]}>
                        <Ionicons name="time-outline" size={11} color={c.tint} />
                        <Text style={[styles.badgeText, { color: c.tint }]}>{weekLabel}</Text>
                      </View>
                    ) : null;
                  })()}
                </View>
              ...
              {reqCounts && !isEditing ? (
                <Animated.View entering={FadeInDown.delay(80).springify()}>
                  <View style={styles.sectionHeaderRow}>
                    ...
                    <Text style={[styles.sectionHint, { color: mutedText }]}>Tap ± to rebalance</Text>
                  </View>
                  <View style={[styles.reqCard, { backgroundColor: cardBg, borderColor: c.border }]}>
                    {(["written_work", "performance_task"] as const).map((field) => {
                      const label = field === "written_work" ? "Written Work" : "Performance Task";
                      const count = reqCounts[field];
                      return (
                        <View key={field} style={[styles.reqRow, { borderBottomColor: c.border }]}>
                          <Text style={[styles.reqLabel, { color: c.text }]}>{label}</Text>
                          <View style={styles.stepperRow}>
                            ...
                            <Animated.Text style={[styles.stepperCount, { color: c.text }, reqScaleStyle]}>
                              {count}
                            </Animated.Text>
                            <Pressable
                    ...
                    <View style={styles.reqRow}>
                      <Text style={[styles.reqLabel, { color: c.text }]}>Exams</Text>
                      <View style={styles.stepperRow}>
                        <View style={[styles.stepperBtn, { borderColor: c.border, backgroundColor: cardBg, opacity: 0.3 }]}>
                          <Ionicons name="remove" size={14} color={c.text} />
                        </View>
                        <Text style={[styles.stepperCount, { color: c.text }]}>{reqCounts.exams}</Text>
                        <View style={[styles.stepperBtn, { borderColor: c.border, backgroundColor: cardBg, opacity: 0.3 }]}>
                          <Ionicons name="add" size={14} color={c.text} />
                        </View>
                      </View>
                    ...
                    {reqBusy ? (
                      <View style={styles.reqOverlay}>
                        <ActivityIndicator color={c.tint} />
                        <Text style={[styles.reqOverlayText, { color: mutedText }]}>Rebalancing…</Text>
                      </View>
                    ) : null}
                  </View>
                </Animated.View>
