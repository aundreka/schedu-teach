...
export function placeOrientation(state: PlacementState): void {
  ...
  // Place orientation in the first usable content slot. nextContentSlot
  // returning null is the capacity gate — if every slot is already claimed
  // by lessons/assessments, orientation gracefully stays unplaced rather
  // than stealing a slot that belongs to required content.
...
export function placeLessonSequence(state: PlacementState): void {
  // Determine upfront whether lessons outnumber usable content slots. When they
  // do, fill each slot to SLOT_CAPACITY before advancing ("front-load"). This
  // keeps earlier slots holding only lower sequence_nos, so placed lessons are
  // always in order. We count only the usable slots from the current scan
  // position so suspended/locked slots don't inflate the available count.
  const usableFromHere = state.plan.content_slot_keys
    .slice(state.contentSlotIndex)
    .filter((key) => {
      const s = state.slotsByKey.get(key);
      return Boolean(s && !s.blackout && !s.is_locked);
    }).length;
  const frontLoad = state.lessons.length > usableFromHere;
  let compressionSlot: RuntimeSlot | null = null;

  while (state.lessons.length > 0) {
    const lesson = state.lessons.shift();
    if (!lesson) return;

    let slot: RuntimeSlot | null = null;

    if (frontLoad && compressionSlot && slotHasRoom(state, compressionSlot)) {
      slot = compressionSlot;
    } else {
      slot = nextContentSlot(state);
    }

    // Out of fresh content slots — compress: ride the lesson along the most
    // recent already-used slot that still has room.
    if (!slot) {
      const candidate = findLessonMergeHost(state);
      // In front-load mode, never place a lesson in a slot that is chronologically
      // earlier than the last placed lesson — that would push a higher sequence_no
      // lesson before a lower one. When no valid slot exists, leave unplaced.
      if (
        candidate &&
        (!frontLoad || !compressionSlot || candidate.slot_date >= compressionSlot.slot_date)
      ) {
        slot = candidate;
        state.warnings.push({
          code: 'BLOCKS_MERGED',
...
export function computeTermBalance(
  ...
  // Exclude buffer blocks (orientation etc.) from the slot-pressure count so that
  // excess_slots here matches the buildBlocks formula — buffer placement is
  // decided separately by placeOrientation using the excessSlots gate.
  const content_blocks = contentBlocks.filter(
    (b) => b.session_category !== 'buffer',
  ).length;
