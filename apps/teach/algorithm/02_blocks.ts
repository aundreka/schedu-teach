...
const EMPTY_ANCHOR: ProgressAnchor = {
  lesson_no: 0,
  written_work_no: 0,
...
function getProgressAnchor(params: BuildBlocksParams): ProgressAnchor {
  const anchor = params.lesson_plan.progress_anchor;
  if (!anchor) return EMPTY_ANCHOR;

  return {
    lesson_no: Math.max(0, anchor.lesson_no ?? 0),
    written_work_no: Math.max(0, anchor.written_work_no ?? 0),
    performance_task_no: Math.max(0, anchor.performance_task_no ?? 0),
    exam_no: Math.max(0, anchor.exam_no ?? 0),
  };
}
...
export function buildBlocks(params: BuildBlocksParams): RuntimeBlock[] {
  const anchor = getProgressAnchor(params);
  const computation = computeBlockComputation(params);
  const terms = getTerms(params);
  const remainingLessons = sortLessons(params.lessons).slice(anchor.lesson_no);
  ...
  let order_no = 1;
  const firstActiveTermIndex = terms.findIndex(
    (term) => term.term.term_no > anchor.exam_no,
  );
  ...
  for (let termIndex = 0; termIndex < terms.length; termIndex += 1) {
    ...
    const termIsCompleted = term.term.term_no <= anchor.exam_no;
    ...
    if (termIndex === firstActiveTermIndex && counts.excess_slots > 0) {
...
export function computeBlockComputation(
  ...
  const anchor = getProgressAnchor(params);
  const terms = getTerms(params);
  const exam_count = Math.max(0, terms.length - anchor.exam_no);
  const total_slots = params.slots.filter(
    (slot) =>
      slot.lesson_plan_id === params.lesson_plan.lesson_plan_id &&
      !slot.blackout,
  ).length;
  const lesson_count = Math.max(0, params.lessons.length - anchor.lesson_no);
  const rawWrittenWorkCount =
    countRuleRequirements(params, 'written_work_count') ??
    params.activities.filter(isWrittenWorkActivity).length;
  const written_work_count = Math.max(0, rawWrittenWorkCount - anchor.written_work_no);
  const rawPerformanceTaskCount =
    countRuleRequirements(params, 'performance_task_count') ??
    params.activities.filter(isPerformanceTaskActivity).length;
  const performance_task_count = Math.max(
    0,
    rawPerformanceTaskCount - anchor.performance_task_no,
  );
...
function distributeCount(
  ...
  const activeIndexes = terms
    .map((term, index) => ({ term, index }))
    .filter(({ term }) => term.term.term_no > skipFirstNTerms)
    .map(({ index }) => index);

  const counts = terms.map(() => 0);
  if (activeIndexes.length === 0 || total <= 0) return counts;

  const base = Math.floor(total / activeIndexes.length);
  let remaining = total - base * activeIndexes.length;
  ...
  const priorityIndexes = priority
    ...
    .filter((index) => index >= 0 && activeIndexes.includes(index));
  ...
  for (let i = activeIndexes.length - 1; remaining > 0 && i >= 0; i -= 1) {
