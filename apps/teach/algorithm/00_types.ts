...
// Per-category counts of items the teacher has already taught/given before the
// plan begins. Used when a teacher onboards mid-term: the algorithm skips the
// first N items in each category and schedules only what remains.
...
export interface LessonPlanRow {
  ...
  // When set, the algorithm treats this (not start_date) as the scheduling
  // floor — slots before this date are not generated. Used for mid-term joins.
  effective_start_date: ISODateString | null;
  progress_anchor: ProgressAnchor | null;
  status: RecordStatus;
  notes: string | null;
  created_at: TimestampString;
  updated_at: TimestampString;
