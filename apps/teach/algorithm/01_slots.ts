...
// The earliest date the algorithm should generate slots for. Defaults to the
// plan's `start_date`, but a teacher onboarding mid-term can set
// `effective_start_date` to today (or later) — anything before that is left
// unscheduled (already-taught territory).
export function getEffectivePlanStart(plan: LessonPlanRow): ISODateString {
  if (!plan.effective_start_date) return plan.start_date;
  return plan.effective_start_date > plan.start_date
...
function expandMeetingPatterns(
  ...
  for (const slot_date of enumerateDates(
    getEffectivePlanStart(params.lesson_plan),
