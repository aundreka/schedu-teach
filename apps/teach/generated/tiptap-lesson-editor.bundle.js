...
3. "-" cannot repeat`);
  ...
  // scripts/tiptap-lesson-editor-entry.ts
  function post(type, payload) {
    var _a;
    (_a = window.ReactNativeWebView) == null ? void 0 : _a.postMessage(JSON.stringify({ type, ...payload }));
  }
  function resolveSafeHeight(element) {
    var _a, _b;
    const candidates = [
      element.scrollHeight,
      element.offsetHeight,
      (_a = document.documentElement) == null ? void 0 : _a.scrollHeight,
      (_b = document.body) == null ? void 0 : _b.scrollHeight
    ].map((value) => Number(value)).filter((value) => Number.isFinite(value) && value > 0);
    const measuredHeight = candidates.length ? Math.max(...candidates) : 0;
    return Math.max(560, Math.ceil(measuredHeight + 24));
  }
  function emitHeight(element) {
    const height = resolveSafeHeight(element);
    post("height", { height });
  }
  function annotateCodeBlocks(root) {
    root.querySelectorAll("pre").forEach((block) => {
      var _a, _b;
      const existingGutter = block.querySelector(":scope > .code-line-gutter");
      const codeElement = block.querySelector("code");
      const content = (_b = (_a = codeElement == null ? void 0 : codeElement.textContent) != null ? _a : block.textContent) != null ? _b : "";
  ...
  function readAlignment(editor) {
    if (editor.isActive({ textAlign: "center" })) return "center";
    if (editor.isActive({ textAlign: "right" })) return "right";
    return "left";
  }
  function emitState(editor) {
    const state = {
  ...
  function syncEditorChrome(editor, element) {
  ...
  function createLessonEditor(config) {
    ...
    const dispatch = (command2) => {
      if (command2.type === "bold") {
        editor.chain().focus().toggleBold().run();
        return;
      }
      if (command2.type === "italic") {
        editor.chain().focus().toggleItalic().run();
        return;
      }
      if (command2.type === "underline") {
        editor.chain().focus().toggleUnderline().run();
        return;
      }
      if (command2.type === "strike") {
        editor.chain().focus().toggleStrike().run();
        return;
      }
      if (command2.type === "bullet") {
        editor.chain().focus().toggleBulletList().run();
        return;
      }
      if (command2.type === "align") {
        editor.chain().focus().setTextAlign(command2.value).run();
        return;
      }
      if (command2.type === "link") {
        const href = command2.url.trim();
        if (!href) return;
        const normalized = /^https?:\/\//i.test(href) ? href : `https://${href}`;
        editor.chain().focus().extendMarkRange("link").setLink({ href: normalized }).run();
        return;
      }
      if (command2.type === "code") {
        editor.chain().focus().toggleCodeBlock().run();
        return;
      }
      if (command2.type === "set-content") {
        editor.commands.setContent(command2.html || "<p></p>", { emitUpdate: false });
      }
    };
    const bridge = {
      dispatch,
